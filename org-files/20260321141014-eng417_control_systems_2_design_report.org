...
:END:
#+title: ENG417 - Control Systems 2 - Design Report 1
...
\newpage
* Question 1
The MATLAB/Octave code for this part can be seen in Appendix A or in the attached file called ~Q1.m~. It was originally written in Octave and tested to work in MATLAB.
...
#+BEGIN_SRC octave :results output :exports none :session EQU_1 :tangle /home/baley/UTAS/ENG417 - Control Systems 2/Q1.m
clc
clear
close all
...
%% For some reason MATLAB fails to solve this, but octave succeeds
[U_eq, W_eq, Q_eq, theta_eq] = solve([eq1, eq2, eq3, eq4], [U, W, Q, theta]);

if exist('OCTAVE_VERSION', 'builtin')
  disp("Common octave W")
else % Hardcode the solutions for MATLAB
  U_eq(1) = U;
  W_eq(1) = W;
  Q_eq(1) = 0;
  theta_eq(1) = asin((-D + R*V*m + T*cos(alpha))/(g*m));

  U_eq(2) = U;
  W_eq(2) = W;
  Q_eq(2) = 0;
  theta_eq(2) = asin((-D + R*V*m + T*cos(alpha))/(g*m));
end
latex(U_eq(2))
latex(W_eq(2))
latex(Q_eq(2))
...
\[\sum_{m=1}^M\frac{\partial f_n}{\partial x_m}|_{x = \bar{x}}(x - \bar{x}_m) = 0 + 0 - Q \bar{W} - g \hdots\]
...
Hence:
\[f_w(\underline{x}) \approx \frac{- L - T \sin{\left(\alpha \right)} + m \left(\hdots\right) - \left(\hdots\right) \left(- D + R V m + T \cos{\left(\alpha \right)}\right) \cos{\left(\phi \right)}}{m}\]
...
If you were to copy the code and replace the equations/constants with the required ones you would find that lateral direction can also be linearised as described in the project sheet. I am not going to show that this is the case because it simply involves copying and replacing the relevant places in the code.

* Question 2
The MATLAB code for this part can be seen in Appendix B or in the attached file called ~Q2.m~.
...
* Question 3
The MATLAB/Octave code for this part can be seen in Appendix C or in the attached file called ~Q3.m~. It was originally written in Octave and tested to work in MATLAB.
...
By subsisting values from the project sheet we get the following matrices for $A$ and $B$.
\[A = \left[\begin{matrix}-1.721 \cdot 10^{-7} & 1.108 \cdot 10^{-6} & 0 & -9.81\\-1.268 \cdot 10^{-5} & -4.203 \cdot 10^{-5} & 182.0 & 0\\1.123 \cdot 10^{-7} & 6.475 \cdot 10^{-8} & -1.076 \cdot 10^{-6} & 0\\0 & 0 & 1.0 & 0\end{matrix}\right]\]
...
\underline{P} &= \left[\begin{matrix}
0.07149  & 0.08363 & 4.983 \cdot 10^{-7} & -1.44 \cdot 10^{-8}  & 0.3004               & \hdots \\
0.4608   & 0       & -5.573              & -1.061 \cdot 10^{-6} & 0.0002471            & \hdots \\
-0.03062 & 0       & 7.08 \cdot 10^{-8}  & 9.394 \cdot 10^{-9}  & -3.609 \cdot 10^{-7} & \hdots \\
0        & 0       & -0.03062            & 0                    & 7.08 \cdot 10^{-8}   & \hdots 
\end{matrix}\right] \\
\textrm{rank}(P) &= 4
\end{align*}
...
CA^{n-1}
\end{bmatrix}\]
If rank of $O_M = n$ then all state variables can be determined based on the output quantities that are measured.
...
\underline{P} &= [B\ AB\ A^2B\ \hdots\ A^{n-1}B] \\
\underline{P} &= \left[\begin{matrix}
0        & 0.08984  & -3.298               & -2.996              & \hdots \\
0.1355   & 0.689    & -1.778 \cdot 10^{-5} & -0.0002842          & \hdots \\
-0.01812 & -0.01646 & 9.895 \cdot 10^{-7}  & 3.146 \cdot 10^{-5} & \hdots \\
0        & 0        & 0.1355               & 0.689               & \hdots 
\end{matrix}\right] \\
\textrm{rank}(P) &= 4
\end{align*}
...
When $B = \left[\begin{matrix}0.08984\\0.689\\-0.01646\\0\end{matrix}\right]$ and $B = \left[\begin{matrix}0\\0.1355\\-0.01812\\0\end{matrix}\right]$ the rank is 4. Only when $B$ is an empty matrix do we get a rank of 0. Hence we either need either $\delta_A$ or $\delta_R$ for the system to be controllable.
...
* Question 4
The MATLAB code for this part can be seen in Appendix D or in the attached file called ~Q4.m~.
...
#+INCLUDE: "/home/baley/UTAS/ENG417 - Control Systems 2/Q1.m" src octave

\newpage
* Appendix B - Code For Question 2

#+INCLUDE: "/home/baley/UTAS/ENG417 - Control Systems 2/ENG417-Project/Q2.m" src octave

\newpage
* Appendix C - Code For Question 3

#+INCLUDE: "/home/baley/UTAS/ENG417 - Control Systems 2/Q1.m" src octave

\newpage
* Appendix D - Code For Question 4

#+INCLUDE: "/home/baley/UTAS/ENG417 - Control Systems 2/ENG417-Project/Q4.m" src octave
