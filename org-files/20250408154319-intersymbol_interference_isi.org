...
We can make $p(t)$ not a square to combat this. This will be done by filtering
 - \[H(f) = H_t(f)H_c(f)H_r(f)\]
   - at $t = nT_s$
** The Niquist zero-ISI Criterion (Pulse Shaping)
To combat ISI.
 - \[R(t) \neq 0, \ t = 0\]
 ...
 - Can be met if we use the Niquist Pulse
   - \[R(t) = \textrm{sinc}(t)\]
     - For $T_s = 1$
 - A more common choice is the raised cosine filter:
   - :TODO:

    
** Causes
*** Multipaths
One path is reflected off the ground, and is LOS. The reflected signal takes an extra $\delta T_s$ to be received. This will bleed into the next signal.
\[r[k] = h[0] x[k] + h[1] x[k-1] + n[k]\]

*** Band-limited Channel
\[r[k] = \sum_{i=0}^{\infty}h[i]x[k-i] + n[k]\]
