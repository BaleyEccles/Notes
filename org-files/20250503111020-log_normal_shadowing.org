...
\[P_r[dBm] = P_t[dBm] - L_p[dB] + X_{\sigma}\]
 - $X_{\sigma} = N\{0, \sigma^2\}$
\[\Pr(P_r(d) > \gamma) = Q\left(\frac{\gamma-\overline{P_r(d)}}{\sigma}\right)\]
* Shadow Fade Margin (SFM)
Distance from the average power to $\gamma$
\[\Pr(\gamma < P_r) = Q\left(\frac{\gamma-P_r}{\sigma}\right) = X\%\]
We can use a [[id:c98340df-6fe8-481d-a2b1-28b7198a62c7][Q table]] to solve this
